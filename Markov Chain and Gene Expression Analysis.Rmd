...
> Replace the NA below with the correct value, we define state 1 as H, state 2 as S0, and state 3 as D.
...
> Using the code from question 1, perform a regression-based analysis to identify genes that are differentially expressed between all normal and tumour samples using Poisson regression. Plot the appropriate -log10 p-values from your analysis.
...
> What other regression methods could you use to identify differentially expressed genes. Give a reason why. Then perform the analysis and plot the appropriate p-values
